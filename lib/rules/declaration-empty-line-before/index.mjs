	...
	return (root, result) => {
		const validOptions = validateOptions(
			...
			},
			{
				...
				possible: {
					except: ['first-nested', 'after-comment', 'after-declaration', 'after-block'],
					ignore: [
						'after-comment',
						'after-declaration',
						'first-nested',
		...
		root.walkDecls((decl) => {
			...
			// Optionally ignore the node if a block precedes it
			if (optionsMatches(secondaryOptions, 'ignore', 'after-block') && isAfterBlock(decl)) {
				return;
			}
 ...
 * @returns {boolean}
 */
function isAfterBlock(node) {
	const prevNode = getPreviousNonSharedLineCommentNode(node);

	return prevNode != null && (isAtRule(prevNode) || isRule(prevNode)) && hasBlock(prevNode);
}
